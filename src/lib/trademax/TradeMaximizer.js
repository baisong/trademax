...
importScripts('javarandom.js');
importScripts('md5.js');

// test input
var testinput = ['(user1) item1 : item2', '(user2) item2 : item1'];
...
class MetricSumSquares {
  ...
  toString() {
    ...
    for (let c of this.counts) str += ' ' + c;
...
class MetricUsersTrading {
  ...
  calculate(cycles) {
    ...
    for (let cycle of cycles) for (let vert of cycle) users[vert.user] = 1;
...
class Entry {
  ...
  decreaseCost(toCost) {
    this.cost = toCost;
    if (this == this.heap.root || this.cost >= this.prev.cost) return;
    if (this == this.prev.child) this.prev.child = this.sibling;
    else this.prev.sibling = this.sibling;
    if (this.sibling != null) this.sibling.prev = this.prev;
    this.prev = null;
...
class Graph {
  ...
  freeze() {
    ...
    for (let v of this.receivers) v.edges = v.edgeList;
    for (let v of this.senders) v.edges = v.edgeList;
  ...
  visitReceivers(receiver) {
    ...
    for (let edge of receiver.edges) {
      ...
      if (v.mark != this.timestamp) this.visitReceivers(v);
    }
  ...
  visitSenders(sender) {
    ...
    for (let edge of sender.edges) {
      ...
      if (v.mark != this.timestamp) this.visitSenders(v);
    }
  ...
  removeBadEdges(v) {
    ...
    for (let edge of v.edges) {
      if (edge.receiver.component == edge.sender.component) newedges.push(edge);
      else edge.sender.dirty = true;
    }
    v.edges = newedges;
  }
  ...
  removeOrphans() {
    ...
    for (let v of this.receivers) {
      if (v.edges.length > 1 || v.edges[0].sender != v.twin)
        newreceivers.push(v);
      else this.orphans.push(v);
    }

    if (newreceivers.length == this.receivers.length) return;
  ...
  findBestMatches() {
    ...
    for (let round = 0; round < this.receivers.length; round++) {
      ...
      for (let v of this.receivers) v.price += v.heapEntry.cost;
      for (let v of this.senders) v.price += v.heapEntry.cost;
    }
  }
  ...
  randomsneeded() {
    ...
    for (let v of this.receivers) count += v.edges.length;
    return count;
  }
  ...
  shuffle() {
    ...
    for (let v of this.receivers) this.shuffleit(v.edges);
  }
  ...
  saveMatches() {
    ...
    for (let v of this.senders) v.savedMatch = v.match;
  }
  ...
  restoreMatches() {
    ...
    for (let v of this.senders) v.match = v.savedMatch;
  }
  ...
  scaleUpEdgeCosts(factor) {
    for (let v of this.senders) {
      v.dirty = true;
      for (let e of v.edges) e.cost *= factor;
    }
  }
  ...
  scaleDownEdgeCosts(factor) {
    for (let v of this.senders) {
      v.dirty = true;
      for (let e of v.edges) e.cost = Math.floor(e.cost / factor);
    }
  }
  ...
  findRequiredEdgesAndShrink(verbose) {
    ...
    if (!verbose) this.tm.output('Shrink (level 1) ');
    ...
    for (run = 2; numRequired > 0; run++) {
      ...
      for (let v of this.receivers) {
        ...
        edgeSet.push(e);
        currentCost += e.cost;
        if (e.status != REQUIRED) e.status = OPTIONAL;
      }
      ...
      for (let i = 0; i < numRequired; i++) {
        ...
        if (edgeSet.includes(e)) requiredEdges[count++] = e;
        else {
          e.status = OPTIONAL;
          e.cost--;
          if (e.cost == e.sender.minimumInCost - 1) e.sender.dirty = true;
    ...
    for (let v of this.receivers) this.removeEdges(v, FORBIDDEN);
    for (let v of this.senders) this.removeEdges(v, FORBIDDEN);
...
*/
    ...
    if (verbose) this.reportStats('Shrink 1 complete', verbose);
  ...
  findForbiddenEdgesAndShrink(verbose) {
    ...
    if (!verbose) this.tm.output('Shrink (level 2) ');
    ...
    for (let v of this.receivers) this.removeEdges(v, UNKNOWN);
    for (let v of this.senders) this.removeEdges(v, UNKNOWN);

    if (verbose) this.reportStats('Shrink level 2 complete', verbose);
    else this.tm.outputln('');
  }

  markEdgesForbiddenIfNotRequired(v) {
    for (let e of v.edges) if (e.status != REQUIRED) e.status = FORBIDDEN;
  }
  ...
  removeEdges(v, statusToRemove) {
    ...
    for (let e of v.edges)
      if (e.status == statusToRemove) e.sender.dirty = true;
  ...
  findUnweightedMatches() {
    for (let v of this.receivers) v.match = null;
    for (let v of this.senders) {
      v.match = null;
      v.price = 0;
    }
    ...
    for (let v of this.receivers) {
      ...
      while (true) {
        ...
        if (i == receiver.edges.length) pos--;
        else {
          let sender = receiver.edges[i].sender;
          if (sender.price == time) continue;

          senderStack[pos] = sender;
          if (sender.match == null) break;
...
class TradeMaximizer {
  ...
  run(input, out, progress, fatal) {
    ...
    if (
      this.priorityScheme != NO_PRIORITIES &&
      (this.metric.length > 1 || this.metric[0].name != 'MetricSumSquares')
    )
      this.outputln(
        'Warning: using priorities with the non-default metric is normally worthless'
      );
    ...
    if (this.showErrors && this.errors.length > 0) {
      ...
      for (let error of this.errors.sort()) this.outputln(error);
      this.outputln('');
    }
...
*/
    ...
    for (let cycle of bestCycles) tradeCount += cycle.length;
...
*/
        ...
        for (let i = 0; i < this.metric.length; i++) {
          ...
          if (value < bestMetric[i]) {
            ...
            foundBetter = true;
            break;
          } else if (value > bestMetric[i]) break;
  ...
  sumOfSquares(cycles) {
    let sum = 0;
    for (let cycle of cycles) sum += cycle.length * cycle.length;
    return sum;
  }
    ...
    for (let lineNumber = 1; ; lineNumber++) {
      ...
      if (line.match('#!.*')) {
        if (wantLists.length > 0)
          this.fatalError(
            'Options (#!...) cannot be declared after first real want list',
            lineNumber
          );
        if (this.officialNames != null)
          this.fatalError(
            'Options (#!...) cannot be declared after official names',
            lineNumber
          );

        let optionslist = line.toUpperCase().substring(2).trim().split(/\s+/);
        for (let option of optionslist) {
          if (option == 'CASE-SENSITIVE') this.caseSensitive = true;
          else if (option == 'REQUIRE-COLONS') this.requireColons = true;
          else if (option == 'REQUIRE-USERNAMES') this.requireUsernames = true;
          else if (option == 'HIDE-ERRORS') this.showErrors = false;
          else if (option == 'HIDE-REPEATS') this.showRepeats = false;
          else if (option == 'HIDE-LOOPS') this.showLoops = false;
          else if (option == 'HIDE-SUMMARY') this.showSummary = false;
          else if (option == 'HIDE-NONTRADES') this.showNonTrades = false;
          else if (option == 'HIDE-STATS') this.showStats = false;
          else if (option == 'SHOW-MISSING') this.showMissing = true;
          else if (option == 'SORT-BY-ITEM') this.sortByItem = true;
          else if (option == 'ALLOW-DUMMIES') this.allowDummies = true;
          else if (option == 'SHOW-ELAPSED-TIME') this.showElapsedTime = true;
            ...
            if (!num.match(/\d+/))
              this.fatalError(
                'SMALL-STEP argument must be a non-negative integer',
                lineNumber
              );
            ...
            if (!num.match(/\d+/))
              this.fatalError(
                'BIG-STEP argument must be a non-negative integer',
                lineNumber
              );
            ...
            if (!num.match(/[1-9]\d*/))
              this.fatalError(
                'NONTRADE-COST argument must be a positive integer',
                lineNumber
              );
            ...
            if (!num.match(/[1-9]\d*/))
              this.fatalError(
                'ITERATIONS argument must be a positive integer',
                lineNumber
              );
            ...
            if (!num.match(/[1-9]\d*/))
              this.fatalError(
                'SEED argument must be a positive integer',
                lineNumber
              );
            ...
            if (!num.match(/[0-9]/)) {
              this.fatalError(
                'SHRINK argument must be a single digit',
                lineNumber
              );
            }
            ...
            for (let m of metrics) {
              ...
              else this.metric.push(metricmap[m]);
            }
          } else this.fatalError('Unknown option "' + option + '"', lineNumber);
      ...
      if (line.match('#.*')) {
        if (line.indexOf('+') == 1)
          // #+ prefixed comments copy to our output
          this.outputln(line);
      ...
      if (line.indexOf('#') != -1)
        if (readingOfficialNames) {
          if (line.split('[:\\s]')[0].indexOf('#') != -1) {
            this.fatalError(
              '# symbol cannot be used in an item name',
              lineNumber
            );
          }
        } else {
          this.fatalError(
            'Comments (#...) cannot be used after beginning of line',
            lineNumber
          );
        }
      ...
      if (line.search(/!BEGIN-OFFICIAL-NAMES/i) != -1) {
        if (this.officialNames != null)
          this.fatalError(
            'Cannot begin official names more than once',
            lineNumber
          );
        if (wantLists.length > 0)
          this.fatalError(
            'Official names cannot be declared after first real want list',
            lineNumber
          );
      ...
      if (line.search(/!END-OFFICIAL-NAMES/i) != -1) {
        if (!readingOfficialNames)
          this.fatalError(
            '!END-OFFICIAL-NAMES without matching !BEGIN-OFFICIAL-NAMES',
            lineNumber
          );
      ...
      if (readingOfficialNames) {
        ...
        if (this.officialNames[name])
          this.fatalError(
            'Official name ' + name + '+ already defined',
            lineNumber
          );
      ...
      if (line.indexOf('(') == -1 && this.requireUsername)
        fatalError(
          'Missing username with REQUIRE-USERNAMES selected',
          lineNumber
        );
      if (line.charAt(0) == '(') {
        ...
        if (line.indexOf(' ') < close)
          line =
            line.substring(0, close + 1).replace(/ /g, '#') +
            ' ' +
      ...
      if (!this.caseSensitive) line = line.toUpperCase();
  ...
  buildGraph(wantLists, longNames) {
    ...
    for (let i = 0; i < wantLists.length; i++) {
      ...
      if (isDummy) {
        if (user == null)
          this.errors.push(
            '**** Dummy item ' + name + ' declared without a username.'
          );
      ...
      if (
        this.officialNames != null &&
        !this.officialNames[name] &&
        name.charAt(0) != '%'
      ) {
        this.errors.push(
          '**** Cannot define want list for ' +
            name +
      ...
      for (let i = 1; i < list.length; i++) {
        ...
        if (toName.indexOf('=') != -1) {
          if (this.priorityScheme != EXPLICIT_PRIORITIES) {
            this.errors.push(
              "**** Cannot use '=' annotation in item " +
                toName +
                ' in want list for item ' +
                fromName +
                ' unless using EXPLICIT_PRIORITIES.'
            );
            continue;
          }
          ...
          if (explicitCost < 1) {
            errors.push(
              '**** Explicit priority must be positive in item ' +
                toName +
                ' in want list for item ' +
                fromName +
                '.'
        ...
        if (toVertex == fromVertex.twin)
          this.errors.push(
            '**** Item ' + toName + ' appears in its own want list.'
          );
          ...
          if (fromVertex.isDummy) cost = this.nonTradeCost;
  ...
  displayMatches(cycles) {
    ...
    if (this.showStats) {
      ...
      for (let groupSize of groupSizes.sort(function (a, b) {
        return b - a;
      }))
    ...
    if (this.graph.randomSequence.length > 0) {
      ...
      if (line.length > 0) all += '#' + line + '\n';
      this.output(all);
    }
  } // displayMatches

  pad(name) {
    while (name.length < this.width) name += ' ';
    return name;
  }
